  ...
  return (
    <section id="gallery" className="py-20 lg:py-28 relative" aria-label="Gallery">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 lg:grid-rows-2 gap-4 lg:gap-6 mt-10">
          {galleryItems.map((item, index) => (
            <motion.div
              ...
              className={`relative rounded-2xl overflow-hidden group cursor-pointer bg-[#1d2b28] ${
                index === 0
            ...
            >
                <Image
                  ...
                  alt={item.alt}
                  fill
                  className={`object-cover transition-transform duration-700 group-hover:scale-110 ${isMobile ? 'auto-scale-pulse' : ''}`}
                  sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw"
                />
                
                {/* Dark overlay - appears on hover */}
                <div className={`absolute inset-0 bg-gradient-to-t from-[#01100c] via-[#01100c]/70 to-transparent opacity-0 group-hover:opacity-90 transition-opacity duration-500 ${isMobile ? 'auto-overlay' : ''}`} />
                
                {/* Content - slides up on hover */}
                <div className={`absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0 ${isMobile ? 'auto-content' : ''}`}>
                  <h3 
                ...
                {/* Bottom indicator */}
                <div className={`absolute bottom-0 left-0 right-0 h-1 bg-[#2ccb99] transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 ${isMobile ? 'auto-indicator' : ''}`} />
            </motion.div>
          ))}
        </div>
      </div>
